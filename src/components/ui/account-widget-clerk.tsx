...
// Widget component that uses Clerk hooks - only loaded when Clerk is configured
export function AccountWidgetWithClerk() {
  const { openSignIn, signOut, openUserProfile } = useClerk();
  const { user } = useUser();
  const { isAnonymous, remainingMessages, messageLimit, usagePercentage } = useUsage();
